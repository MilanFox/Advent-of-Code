...
const digSiteSize = Object.values(inputData).reduce((acc, cur) => {
  ...
  Object.entries(directions).forEach(([key, value]) => {
    if (cur.dir === key) newSize = {
      width: acc.width + cur.length * value.offsetX,
      height: acc.height + cur.length * value.offsetY
    };
  })
  return {
    width: newSize.width,
...
const digSite = Array.from({ length: digSiteSize.maxHeight + Math.abs(digSiteSize.minHeight) + 1 + (mapPadding * 2) }, () =>
  Array.from({ length: digSiteSize.maxWidth + Math.abs(digSiteSize.minWidth) + 1 + (mapPadding * 2) }, () => ' '));

const dig = ({list, y, x}) => {
  const diggerPosition = { y: y + Math.abs(digSiteSize.minHeight) + mapPadding, x: x + Math.abs(digSiteSize.minWidth) + mapPadding };
