...
const buildFullGraph = () => {
  Object.keys(inputData).forEach(node => {
    inputData[node].connected.forEach(connectionNode => {
      if (!inputData[connectionNode]) inputData[connectionNode] = {name: connectionNode, connected: new Set([node])}
      else inputData[connectionNode].connected.add(node);
    })
  })
}

// Pathfinding inspired by https://hackernoon.com/a-beginners-guide-to-bfs-and-dfs-in-javascript
...
/*
   For a lack of better ideas, going with pure "guestimation" here:
